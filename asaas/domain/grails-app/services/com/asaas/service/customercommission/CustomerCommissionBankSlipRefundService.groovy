...
@GrailsCompileStatic
@Transactional
class CustomerCommissionBankSlipRefundService {
    ...
    public void createCommissions() {
        ...
        List<Map> pendingCreateCustomerCommissionAsyncActionList = customerCommissionAsyncActionService.listPending(CustomerCommissionAsyncActionType.CREATE_BANK_SLIP_FEE_REFUND_CUSTOMER_COMMISSION, maxItemsPerCycle)
        ...
        Utils.forEachWithFlushSession(pendingCreateCustomerCommissionAsyncActionList, 50, { Map asyncActionData ->
            ...
            Utils.withNewTransactionAndRollbackOnError({
                ...
                Boolean hasPendingBankSlipFeeCreditToBeProcessed = customerCommissionAsyncActionService.hasPendingCreditCustomerCommission(commissionedAccountId, CustomerCommissionAsyncActionType.CREATE_BANK_SLIP_FEE_CUSTOMER_COMMISSION)
                if (hasPendingBankSlipFeeCreditToBeProcessed) return
                ...
                if (Utils.isLock(exception)) {
                    AsaasLogger.warn("CustomerCommissionBankSlipRefundService.createCommissions >> Lock ao processar a AsyncAction de comissão [${asyncActionId}]")
                    return
                }

                AsaasLogger.error("CustomerCommissionBankSlipRefundService.createCommissions >> Erro ao criar estorno de comissão AsyncActionData: ${asyncActionData}")
    ...
    private void processCustomerCommissionAsyncAction(Customer ownedAccount, Date transactionDate, CustomerCommissionConfig config, Long asyncActionId) {
        CustomerCommission customerCommission = customerCommissionService.findExistingCustomerCommissionWithSameParameters(config.customer, ownedAccount, transactionDate, CustomerCommissionType.BANK_SLIP_FEE_REFUND)
        if (customerCommission?.status?.isItemCreationDone()) {
            customerCommissionAsyncActionService.delete(asyncActionId)
            return
        }

        CreateCustomerCommissionRefundItemAdapter commissionRefundAdapter = new CreateCustomerCommissionRefundItemAdapter(ownedAccount, transactionDate, CustomerCommissionType.BANK_SLIP_FEE_REFUND, BillingType.equivalentToBoleto(), CustomerCommissionType.BANK_SLIP_FEE)
        if (commissionRefundAdapter.financialTransactionMapList) {
            if (!customerCommission) customerCommission = customerCommissionService.save(config.customer, ownedAccount, transactionDate, CustomerCommissionType.BANK_SLIP_FEE_REFUND)
