...
@Transactional
class FinancialStatementDirectPixPrecautionaryBlockService {
    ...
    public void createFinancialStatements(Date startDate, Date endDate) {
        ...
        AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createFinancialStatements >> [início: ${new Date()}]")

        StopWatch stopWatch = new StopWatch("FinancialStatementDirectPixPrecautionaryBlockService")

        for (Date date in startDate..endDate) {
            Utils.withNewTransactionAndRollbackOnError({
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskConfirmed:${date} >> [início: ${new Date()}]")
                stopWatch.start("createForPixCashInRiskConfirmed:${date}")
                createForPixCashInRiskConfirmed(date)
                stopWatch.stop()
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskConfirmed:${date} >> [conclusão: ${new Date()}]")
            }, [logErrorMessage: "FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskConfirmed >> Erro ao executar da data ${date}."])

            Utils.flushAndClearSession()

            Utils.withNewTransactionAndRollbackOnError({
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskRefunded:${date} >> [início: ${new Date()}]")
                stopWatch.start("createForPixCashInRiskRefunded:${date}")
                createForPixCashInRiskRefunded(date)
                stopWatch.stop()
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskRefunded:${date} >> [conclusão: ${new Date()}]")
            }, [logErrorMessage: "FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskRefunded >> Erro ao executar da data ${date}."])

            Utils.flushAndClearSession()

            Utils.withNewTransactionAndRollbackOnError({
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskReceived:${date} >> [início: ${new Date()}]")
                stopWatch.start("createForPixCashInRiskReceived:${date}")
                createForPixCashInRiskReceived(date)
                stopWatch.stop()
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskReceived:${date} >> [conclusão: ${new Date()}]")
            }, [logErrorMessage: "FinancialStatementDirectPixPrecautionaryBlockService.createForPixCashInRiskReceived >> Erro ao executar da data ${date}."])
            ...
            Utils.withNewTransactionAndRollbackOnError({
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForReceivedAfterFailedRefundByAnalysis:${date} >> [início: ${new Date()}]")
                stopWatch.start("createForReceivedAfterFailedRefundByAnalysis:${date}")
                createForReceivedAfterFailedRefundByAnalysis(date)
                stopWatch.stop()
                AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createForReceivedAfterFailedRefundByAnalysis:${date} >> [conclusão: ${new Date()}]")
            }, [logErrorMessage: "FinancialStatementDirectPixPrecautionaryBlockService.createForReceivedAfterFailedRefundByAnalysis >> Erro ao executar da data ${date}."])

            Utils.flushAndClearSession()
        }

        AsaasLogger.info(stopWatch.prettyPrint())
        AsaasLogger.info("FinancialStatementDirectPixPrecautionaryBlockService.createFinancialStatements >> [conclusão: ${new Date()}]")
    }
    ...
    private void createForPixCashInRiskConfirmed(Date date) {
        ...
        query.setParameterList("financialStatementTypeList", [FinancialStatementType.PIX_DIRECT_CASH_IN_RISK_CUSTOMER_REVENUE.toString(), FinancialStatementType.PIX_CASH_IN_RISK_TRANSITORY_DEBIT.toString()])
        ...
        Map financialStatementCustomerRevenueInfoMap = [
            financialStatementType: FinancialStatementType.PIX_DIRECT_CASH_IN_RISK_CUSTOMER_REVENUE,
    ...
    private void createForPixCashInRiskRefunded(Date date) {
        ...
        query.setBoolean("fsDeleted", false)
        query.setParameterList("financialStatementTypeList", [FinancialStatementType.PIX_DIRECT_CASH_IN_RISK_CUSTOMER_REVENUE_REVERSAL.toString(), FinancialStatementType.PIX_CASH_IN_RISK_REFUNDED_TRANSITORY_CREDIT.toString()])
        ...
        Bank bank = Bank.findByCode(SupportedBank.ASAAS.code())
        Map financialStatementCustomerRevenueInfoMap = [
            financialStatementType: FinancialStatementType.PIX_DIRECT_CASH_IN_RISK_CUSTOMER_REVENUE_REVERSAL,
            totalValue: paymentInfoList.value.sum()
        ]
        financialStatementService.saveForPaymentIdList(paymentInfoList.collect { it.id } as List<Long>, [financialStatementCustomerRevenueInfoMap], date, bank)
